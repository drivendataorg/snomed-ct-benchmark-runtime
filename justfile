...
image_name := "snomed-ct-benchmark-runtime"
official_image := "snomedctbenchmarkprodacr.azurecr.io/" + image_name
...
# faiss-builder image settings
faiss_version := "v1.13.2"
faiss_image := "faiss-builder:" + faiss_version
faiss_acr_image := "snomedctbenchmarkprodacr.azurecr.io/faiss-builder:" + faiss_version

# Pull faiss-builder from ACR (fast, recommended)
[group('faiss')]
pull-faiss:
    docker pull {{faiss_acr_image}}
    docker tag {{faiss_acr_image}} {{faiss_image}}

# Build faiss-builder locally (slow, only if you can't pull from ACR)
[group('faiss')]
build-faiss:
    docker build runtime \
        -f runtime/Dockerfile.faiss \
        --tag {{faiss_image}}

# Push faiss-builder to ACR (requires Azure CLI login and LOGIN_SERVER set)
# Usage: set -a && source .env && set +a && just push-faiss
[group('faiss')]
_push-faiss:
    #!/usr/bin/env bash
    set -euo pipefail
    if [[ -z "${LOGIN_SERVER:-}" ]]; then
        echo "Error: Missing LOGIN_SERVER. Source your .env file first:"
        echo "  set -a && source .env && set +a && just push-faiss"
        exit 1
    fi
    echo "Logging into ACR..."
    ACR_NAME="${ACR_NAME:-${LOGIN_SERVER%%.azurecr.io}}"
    TOKEN="$(az acr login --name "$ACR_NAME" --expose-token --output tsv --query accessToken)"
    echo "$TOKEN" | docker login "$LOGIN_SERVER" -u 00000000-0000-0000-0000-000000000000 --password-stdin
    echo "Tagging image..."
    docker tag {{faiss_image}} {{faiss_acr_image}}
    echo "Pushing to ACR..."
    docker push {{faiss_acr_image}}
    echo "Done! Pushed {{faiss_acr_image}}"

# Build the runtime container (requires faiss-builder image)
[group('development')]
build:
    docker build runtime \
        {{platform_args}} \
        --target runtime \
        --build-context faiss-builder=docker-image://{{faiss_image}} \
        --tag {{local_image}}:{{local_tag}}

# Build the test container (requires faiss-builder image)
[group('tests')]
build-tests:
    docker build runtime \
        {{platform_args}} \
...
# Generate smoke test data from training notes and annotations
[group('* test submission locally')]
smoke-test-data:
    uv run scripts/smoke_test_data.py data/train_notes.csv data/train_annotations.csv
